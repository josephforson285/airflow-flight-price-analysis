...
assert_eq "$(mysql_scalar 'SELECT COUNT(*) FROM raw_flight_prices')" 27 "rows landed"
assert_eq "$(mysql_scalar 'SELECT COUNT(DISTINCT raw_row_num) FROM rejects_flight_prices')" 13 "rows quarantined"
assert_eq "$(mysql_scalar 'SELECT COUNT(DISTINCT reason_code) FROM rejects_flight_prices')" 11 "distinct reject reasons"
assert_eq "$(mysql_scalar 'SELECT COUNT(*) FROM stg_flights')" 14 "clean rows promoted"
...
assert_eq "$(pg_scalar 'SELECT COUNT(*) FROM fct_flights')" 14 "analytics untouched by blocked run"
